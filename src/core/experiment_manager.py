...
class ExperimentManager:
    ...
    def get_dataset_builder_for_task(self):
        ...
        """
        dataset_name = get_config(self.config, "dataset.name", "default")
    ...
    def setup_test_data(self):
        """
        Build a clean test loader.
        """
        dataset_name = get_config(self.config, "dataset.name", "default")
        builder = get_dataset_builder(dataset_name)(self.config)
    ...
    def setup_trainer(self) -> None:
        """Setup trainer for segmentation tasks (single-task project)."""
        # evaluation strategy
        ...
        method = get_config(self.config, "training.method", "baseline")
        trainer_cls = get_trainer(method)   # from src.training.trainer_registry import get_trainer

        self.trainer = trainer_cls(self.config, self.device, evaluation_strategy)
        self.trainer.setup(
            self.model,
            self.criterion,
            self.optimizer,
        ...
        self.logger.info(f"Trainer created: {type(self.trainer).__name__}, method={method}")
